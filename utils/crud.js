...
const pushToDatabase = async (word) => {
    await wordModel
        ...
        .then(async (response) => {
            if (response) {
                console.log(
                    "this word is already present in the database.\npress <ctrl-c> to quit.",
                );
            } else {
...
const viewDatabase = async () => {
    try {
        const db = await wordModel
            .find({})
            .then((data) => {
                if (!data.length) {
                    console.log(
                        "oops! looks like there is nothing in the database, try adding something with the add command.\nFor example: words add food",
                    );
                } else {
